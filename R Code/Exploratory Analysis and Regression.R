...
circle_staydays <- ggmap(map) + geom_point(aes(x = longitude, y = latitude, size = (stayzilla1$adults*1.2)),data = stayzilla1,col = "blue", alpha = .5, shape = 1, stroke =2)

circle_staydays +
  stat_density2d(aes(x = longitude, y = latitude, fill = ..level..,alpha=..level..), bins = 10, geom = "polygon", data = stayzilla1) +
